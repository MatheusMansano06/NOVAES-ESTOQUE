...
export const useEstoqueMinimo = () => {
    ...
    const criar = async (produto_codigo, estoque_minimo, notificar_fornecedores) => {
        setLoading(true);
        setError(null);
        try {
    ...
    const editar = async (produto_codigo, estoque_minimo, notificar_fornecedores) => {
        setLoading(true);
        setError(null);
        try {
            ...
            setConfigs(prev => prev.map(c => c.produto_codigo === produto_codigo ? response.data.config : c));
            return response.data.config;
        }
        catch (err) {
