...
> **Nota do port (15/07/2026).** Este documento veio do projeto DEVOLUCOES-ML-main
> (Flask). A função vive agora em **`backend/app/devolucoes_ml.py`**, portada
> VERBATIM — mesmo sha1 do corpo original, verificado. A referência abaixo dizia
> `app.py:1441`, mas no projeto de origem a função já estava em `app.py:1671`: a
> linha citada estava defasada. Não confie no número da linha; procure pelo nome.
>
> **A suíte `tests/test_ml_contract.py` do projeto de origem está DEFASADA** e não
> serve de referência: rodada contra o próprio código original dela, 2 testes
> falham — `test_classifier_accepts_closed_touched_with_return_delivered` espera
> bucket `outros_problemas`/`closed_touched_with_return_delivered`, enquanto as
> regras vigentes devolvem `fora_da_fila`/`closed_resolved_not_next_attention`.
> O teste reflete uma versão antiga das regras. A equivalência do port foi
> provada por teste diferencial (1.728.000 combinações de entrada, 0 divergências,
> 16 regras distintas atingidas), não por essa suíte.

## O que está congelado?

A função `classify_ml_live_queue_claim()` em `backend/app/devolucoes_ml.py`
